...
export default function SettingsPage() {
  ...
  const handleAdd = async () => {
    ...
    if (providerInfo && providerInfo.required_keys) {
      for (const key of providerInfo.required_keys) {
        const value = formData[key as keyof UserConfigRequest];
  ...
  const handleToggle = async (config: UserConfig) => {
    ...
    try {
      let res;
      const updateData: UserConfigRequest = {
        name: config.name,
      ...
      switch (activeTab) {
        case 'llm':
          res = await userConfigApi.updateLLMConfig(config.id, updateData);
          break;
        case 'search':
          res = await userConfigApi.updateSearchConfig(config.id, updateData);
          break;
        case 'asr':
          res = await userConfigApi.updateASRConfig(config.id, updateData);
          break;
        case 'embedding':
          res = await userConfigApi.updateEmbeddingConfig(config.id, updateData);
          break;
      }
                      ...
                      <>
                        {getConfigFields().map(field => {
                          ...
                          const setValue = (val: string) => {
                            const updates: Partial<UserConfigRequest> = {};
                            if (field === 'api_key') updates.api_key = val;
                            else if (field === 'api_url') updates.api_url = val;
                            else if (field === 'model') updates.model = val;
                            else if (field === 'dimensions') updates.dimensions = parseInt(val) || 1536;
                            else if (field === 'daily_quota') updates.daily_quota = parseInt(val) || 100;
